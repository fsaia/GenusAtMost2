// Here, we work to determine equations for non-elliptic genus 1 quotients
// X_0^D(N)/W with W non-trivial, as was done in Gonzalez--Rotger 2006 in the case
// of N squarefree and |W| = 2. 

load "genus_1_AL_quotients_no_rat_pts.m";
load "genus_0_AL_quotients_rat_pts.m";
load "genus_1_AL_quotient_jacobian_isomorphism_classes.m";
load "AL_identifiers.m";

// function for sorting elements of non_elliptic_genus_1_equations_determined 
// after each check
...
// initializing lists for those non-elliptic genus one quotients X_0^D(N)/W
// for which we determine an equation, and those for which we determine more than
// one candidate equations
non_elliptic_genus_1_equations_determined := [];
non_elliptic_genus_1_equation_not_determined_initial := [];

// initializing list to keep track of m0 values used to generate
// candidate hyperelliptic models (i.e., the w_{m0} serving as the hyperelliptic 
// involution)
non_elliptic_genus_1_with_m0 := [];
...
non_elliptic_genus_1_lacking_CM := [];
...
for T in [T : T in genus_1_AL_quotients_no_rat_pts | IsSquarefree(T[2])] do 
    print T;
    D := T[1];
    N := T[2];
    gens := T[3];
    ...
    if not (IsEmpty(m0_disc_pair)) then 
        m0 := m0_disc_pair[1];
        d := m0_disc_pair[2];
        Append(~non_elliptic_genus_1_with_m0,[*D,N,gens,m0*]);
        ...
        if #H eq 1 then 
            Append(~non_elliptic_genus_1_equations_determined,[*D,N,gens,H[1]*]);
        else
            Append(~non_elliptic_genus_1_equation_not_determined_initial,[*D,N,gens,H*]);

        end if; 

    else
        Append(~non_elliptic_genus_1_lacking_CM,[*D,N,gens*]);
    end if; 

end for; 


SetOutputFile("non_elliptic_genus_1_equations_determined_initial.m");
print "_<x> := PolynomialRing(Rationals());";
print "non_elliptic_genus_1_equations_determined_initial := ";
print non_elliptic_genus_1_equations_determined;
print ";";
UnsetOutputFile();

SetOutputFile("non_elliptic_genus_1_equation_not_determined_initial.m");
print "_<x> := PolynomialRing(Rationals());";
print "non_elliptic_genus_1_equation_not_determined_initial := ";
print non_elliptic_genus_1_equation_not_determined_initial;
print ";";
UnsetOutputFile();

SetOutputFile("non_elliptic_genus_1_lacking_CM.m");
print "non_elliptic_genus_1_lacking_CM := ";
print non_elliptic_genus_1_lacking_CM;
print ";";
UnsetOutputFile();

SetOutputFile("non_elliptic_genus_1_with_m0.m");
print "non_elliptic_genus_1_with_m0 := ";
print non_elliptic_genus_1_with_m0;
print ";";
UnsetOutputFile();


// trying to use local obstruction checks to further exclude candidate equations 
// in the cases where they were not uniquely determined

load "dual_graphs.m";
non_elliptic_genus_1_equation_not_determined_local_points := [];
print "running local obstructions checks...";

for X in non_elliptic_genus_1_equation_not_determined_initial do 
    print X;
    D := X[1];
    N := X[2];
    gens := X[3];
    ...
    if #H eq 1 then 
        ...
        Append(~non_elliptic_genus_1_equation_not_determined_local_points,[*D,N,gens,H*]);
    end if;
end for;

Sort(~non_elliptic_genus_1_equations_determined,sort_non_elliptics);

SetOutputFile("non_elliptic_genus_1_equations_determined_local_points.m");
print "_<x> := PolynomialRing(Rationals());";
print "non_elliptic_genus_1_equations_determined_local_points := ";
print non_elliptic_genus_1_equations_determined;
print ";";
UnsetOutputFile();

SetOutputFile("non_elliptic_genus_1_equation_not_determined_local_points.m");
print "_<x> := PolynomialRing(Rationals());";
print "non_elliptic_genus_1_equation_not_determined_local_points := ";
print non_elliptic_genus_1_equation_not_determined_local_points;
print ";";
UnsetOutputFile();
...
non_elliptic_genus_1_equation_not_determined_real_points := [];

// Each of the remaining curves in the local points checked list has exactly 2 candidate equations remaining. 
// If one of these has real points and the other does not, then we use real point checks to try to
// determine which is correct. 
for X in non_elliptic_genus_1_equation_not_determined_local_points do
    D := X[1];
    N := X[2];
    gens := X[3];
    H := X[4];
    ...
    if #H eq 1 then 
        ...
        Append(~non_elliptic_genus_1_equation_not_determined_real_points,[*D,N,gens,H*]);
...
SetOutputFile("non_elliptic_genus_1_equations_determined_real_points.m");
print "non_elliptic_genus_1_equations_determined_real_points := ";
print non_elliptic_genus_1_equations_determined;
print ";";
UnsetOutputFile();

SetOutputFile("non_elliptic_genus_1_equation_not_determined_real_points.m");
print "non_elliptic_genus_1_equation_not_determined_real_points := ";
print non_elliptic_genus_1_equation_not_determined_real_points;
print ";";
UnsetOutputFile();
